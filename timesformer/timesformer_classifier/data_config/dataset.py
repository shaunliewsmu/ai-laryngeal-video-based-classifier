...
class VideoDataset(Dataset):
    def __init__(self, root_dir, mode='train', sampling_method='uniform', 
        ...
        """
        Initialize the dataset for TimeSformer model training with pytorchvideo samplers.
    ...
    def _extract_clip(self, container, clip_info):
        """Extract video clip based on clip_info with proper format handling."""
        ...
        for frame in container.decode(video=0):
            if frame_count >= next_frame_to_extract:
                ...
                # Ensure frame has correct dimensions (H, W, 3)
                if len(img.shape) == 2:  # Grayscale image
                    img = np.stack([img, img, img], axis=2)
                elif img.shape[0] == 3 and len(img.shape) == 3 and img.shape[2] != 3:  # Channel-first format
                ...
                # Ensure frame has correct data type
