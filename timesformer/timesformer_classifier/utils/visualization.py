...
class TrainingVisualizer:
    ...
    def visualize_sampling(self, video_path, sampling_method, num_frames, save_path, title=''):
        ...
        # Calculate dynamic FPS if needed
        dynamic_fps = None
        ...
        frames_info = f"{len(indices)} frames from {total_frames} total{fps_info}"
            ...
            try:
                ...
                if isinstance(video, np.ndarray):
                    ...
                    if len(video.shape) == 4:  # (T, H, W, C)
                        ...
                        self.logger.warning(f"Unexpected video shape: {video.shape}")
                        continue
                else:
                ...
                self.logger.error(f"Error plotting sample {i}: {str(e)}")
                continue
