...
class EnhancedVisualizer:
    """Enhanced visualization class for creating detailed training visualizations."""
    ...
    def plot_training_history(self, history):
        """
        Plot training and validation metrics over epochs with enhanced details.
        ...
        # Add AUROC plot if available
        if 'train_auroc' in history and 'val_auroc' in history:
            fig.set_figheight(15)
            ax3 = fig.add_subplot(3, 1, 3)
            ax3.plot(history['train_auroc'], label='Training AUROC', color='#FF5733')
            ax3.plot(history['val_auroc'], label='Validation AUROC', color='#33A2FF')
            ax3.set_xlabel('Epoch')
            ax3.set_ylabel('AUROC')
            ax3.set_title('Training and Validation AUROC')
            ax3.legend()
            ax3.grid(True, alpha=0.3)
            
            # Add best AUROC marker if epochs > 1
            if len(history['val_auroc']) > 1:
                best_auroc_epoch = np.argmax(history['val_auroc'])
                ax3.axvline(x=best_auroc_epoch, color='green', linestyle='--', alpha=0.5,
                          label=f'Best AUROC (epoch {best_auroc_epoch+1})')
                ax3.plot(best_auroc_epoch, history['val_auroc'][best_auroc_epoch], 'go', markersize=8)
    ...
    def plot_confusion_matrix(self, conf_matrix, class_names=None):
        """
        Plot confusion matrix with percentages and metrics.
        ...
        class_names = class_names or ['non_referral', 'referral']
    ...
    def plot_sample_predictions(self, videos, true_labels, pred_labels, class_names=None):
        """
        Plot sample video frames with their true and predicted labels.
        
        Args:
            videos (torch.Tensor): Batch of videos (B, C, T, H, W)
        ...
        num_samples = min(len(videos), 5)  # Show up to 5 samples
        ...
        for i in range(num_samples):
            try:
                ...
                if len(video.shape) == 4:  # (C, T, H, W)
                    T = video.shape[1]
                else:  # Handling other potential shapes
                    video = video.reshape(-1, video.shape[0], video.shape[1], video.shape[2])
                    T = video.shape[0]
                ...
                for j, frame_idx in enumerate(frame_indices):
                    ...
                    else:  # Other potential shapes
                        frame = video[frame_idx]  # Shape might vary
                    ...
                    # Ensure frame is in the right format for matplotlib (H, W, C)
                    if frame.shape[0] == 3:  # If channels first (C, H, W)
                ...
                # Get labels
                true_label = class_names[int(true_labels[i])] if class_names else true_labels[i]
                pred_label = class_names[int(pred_labels[i])] if class_names else pred_labels[i]
    ...
    def visualize_sampling(self, video_path, sampling_method, num_frames, save_path, title=''):
        ...
        if total_frames < num_frames:
            dynamic_fps = num_frames / video_duration
            method_name = f"{sampling_method.replace('_', ' ').title()} (Dynamic FPS: {dynamic_fps:.2f})"
        else:
        ...
        if dynamic_fps:
            frames_info += f" (Dynamic FPS: {dynamic_fps:.2f})"
    ...
    def plot_auroc_curve(self, y_true, y_score, save_path=None):
        """
        Plot ROC curve with optimal threshold point.
        
        Args:
            y_true (array-like): True binary labels
            y_score (array-like): Target scores (probability estimates)
